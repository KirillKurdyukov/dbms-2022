...
-- Косая запись нам не грозит, так как никакие данные не обновляются.
...
-- Обнаружил ошибку в своей реализации reverse, что идет конфликт в insert'e.
-- Когда обновляется истекшая бронь.
-- Но upsert / update атомарен, поэтому неповторяемое чтение не грозит, как и косая запись,
-- так как инвариант отсутствует.
